...
multilevel_pca = function(Y = NULL, id = NULL, visit=NULL, twoway = FALSE,
  ...
  if (twoway) {
    ...
    for (j in 1:J) {
      Y.split[[j]] = base::subset(Y.df, visit == j)
  ...
  if(cov.method == "m1"){
    Gt = matrix(0, N, N)
    for(m in 1:M){
      Y.tilde.m = base::subset(Y.df.new, id == uni.id[m])$Y.tilde
  ...
  total.variance <- sum(fpca1.value) + sum(fpca2.value)
  if (is.na(K1) | is.na(K2)){  # if no values for the numbers of principal components N.X and N.U are specified:
    prop <- K1 <- K2 <- 0
    while(prop < pve){              # add components for X or U with decreasing variance,
      # until level L of explained average variance is reached
      if (fpca1.value[K1 + 1] >= fpca2.value[K2 + 1]){
        K1 <- K1 + 1
      }
      else {
        K2 <- K2 + 1
      }
      prop <- (sum(fpca1.value[1:K1]) + sum(fpca2.value[1:K2])) / total.variance  # update explained average variance
    }
  }
  explained.variance <- (sum(fpca1.value[1:K1]) + sum(fpca2.value[1:K2])) / total.variance  # explained average variance 
  percent1 = (fpca1.value)/total.variance
  percent2 = (fpca2.value)/total.variance 
  ...
  efunctions = list(fpca1.vectors[,1:K1], fpca2.vectors[,1:K2])
  scores = list(s1, s2)
  pctvar = list(percent1[1:K1], percent2[1:K2])
  varofTot = list(px, pw)
  Y.df = Y.df.new[,-4]
  
  names(efunctions) = names(evalues) = names(npc) = names(scores) = names(pctvar) = names(varofTot) = c("level1", "level2")
  ret.objects = c("Y_raw","Y.df", "mu", "eta","npc", "pctvar","varofTot","efunctions", "evalues", "scores")
